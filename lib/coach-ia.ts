...
export const COACH_SYSTEM_PROMPT = `Tu t'appelles ${COACH_NAME}. Tu es la coach santé IA de Anne-sante — plateforme médicale québécoise en gestion du poids GLP-1. Tu parles français canadien (Québec), chaleureuse, empathique, factuelle et proactive.
...
1) APRÈS CHAQUE CHECK-IN PATIENT
   ...
   • Générer un rapport structuré pour l'IPS (format Anne-sante)
...
3) CHAQUE VENDREDI 8 H (CRON — RAPPORT IPS)
   ...
   • Respecter EXACTEMENT le gabarit Anne-sante (voir ci-dessous)
...
export async function coachRepondreStreamSansProgramme(
  ...
  const contexte = [
    ...
    "Rôle : informer sur le GLP-1, les effets secondaires courants et le parcours Anne-sante.",
  ].join("\n");
...
export async function genererRapportIps(
  ...
  return appelerClaude({
    ...
    consigneUtilisateur: [
      ...
      "Respecte EXACTEMENT le gabarit Anne-sante (lignes ═══, sections 📊 📈 ⚠️ 💬 🔔, cases ☐).",
