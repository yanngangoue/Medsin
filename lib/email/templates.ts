...
export function welcomeEmail(prenom: string) {
  ...
  const subject = "Bienvenue sur Anne-sante";
  const html = layout(`
    <h1 style="font-size:20px">Bonjour ${prenom},</h1>
    <p>Votre compte Anne-sante est créé. Vous pouvez poursuivre votre parcours de santé à tout moment.</p>
...
export function eligibilityDecisionEmail(
  ...
  const subject = `Anne-sante — mise à jour de votre dossier (${label})`;
...
export function appointmentConfirmedEmail(
  ...
  const subject = `Anne-sante — consultation vidéo planifiée (${formattedDate})`;
  const html = layout(`
    <h1 style="font-size:20px">Bonjour ${prenom},</h1>
...
export function appointmentReminderEmail(
  ...
  const base = getAppBaseUrl();
  const joinUrl = `${base}/dashboard/patient/consultation/${appointmentId}`;
  const subject = `Rappel Anne-sante — visio ${windowLabel} (${formattedDate})`;
  const html = layout(`
    <h1 style="font-size:20px">Bonjour ${prenom},</h1>
    <p>Rappel : votre consultation vidéo Anne-sante a lieu <strong>${windowLabel}</strong> (${formattedDate}).</p>
