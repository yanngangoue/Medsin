...
import { getStripeIpeWebhookSecret } from "@/lib/stripe/env";
...
export async function processStripeWebhook(req: Request): Promise<NextResponse> {
  const webhookSecret = getStripeIpeWebhookSecret();
  if (!webhookSecret) {
    return NextResponse.json(
      { error: "Webhook non configuré — STRIPE_IPE_WEBHOOK_SECRET manquant", code: "SERVICE_UNAVAILABLE" },
      { status: 503 },
    );
  }
  ...
  if (!signature) {
    return NextResponse.json(
      { error: "Signature manquante", code: "VALIDATION_ERROR" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Signature invalide", code: "VALIDATION_ERROR" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Traitement échoué", code: "INTERNAL_ERROR" },
      { status: 500 },
    );
  }
