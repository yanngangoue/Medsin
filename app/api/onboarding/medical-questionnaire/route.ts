...
export async function PATCH(req: Request) {
  return catchRouteError("onboarding/medical-questionnaire/PATCH", async () => {
    const session = await auth();
      if (!session?.user?.id) {
        return NextResponse.json({ error: "Non autorisé", code: "UNAUTHORIZED" }, { status: 401 });
      }
...
export async function POST(req: Request) {
  return catchRouteError("onboarding/medical-questionnaire/POST", async () => {
    const session = await auth();
      if (!session?.user?.id) {
        return NextResponse.json({ error: "Non autorisé", code: "UNAUTHORIZED" }, { status: 401 });
      }
      ...
      if (user) {
        await sendEmail({
          to: user.email,
          subject: "Votre dossier Anne-sante est en cours d'examen",
      ...
      if (ipsId) {
        ...
        if (ips) {
          ...
          await sendEmail({
            to: ips.email,
            subject: `Nouveau patient à réviser — ${user?.prenom ?? "Patient"}`,
