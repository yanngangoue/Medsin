...
import { sleepIntakeSchema } from "@/lib/metabolic/schemas";
import { fhirObservationSleep } from "@/lib/metabolic/fhir-normalize";
import { persistMetabolicIntake, MetabolicConsentError } from "@/lib/metabolic/record-intake";

export async function POST(req: NextRequest) {
  return catchRouteError("interop/v1/metabolic/intake/sleep/POST", async () => {
    if (!parseTenantFromHeaders(req.headers)) {
      ...
      const parsed = sleepIntakeSchema.safeParse(json);
      if (!parsed.success) {
        return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
      }
      try {
        const fhir = fhirObservationSleep({ patientUserId: principal.userId, ...parsed.data });
