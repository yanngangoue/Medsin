...
import { supplementIntakeSchema } from "@/lib/metabolic/schemas";
import { fhirNutritionSupplement } from "@/lib/metabolic/fhir-normalize";
import { persistMetabolicIntake, MetabolicConsentError } from "@/lib/metabolic/record-intake";

export async function POST(req: NextRequest) {
  return catchRouteError("interop/v1/metabolic/intake/supplement/POST", async () => {
    if (!parseTenantFromHeaders(req.headers)) {
      ...
      const parsed = supplementIntakeSchema.safeParse(json);
      if (!parsed.success) {
        return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
      }
      try {
        const fhir = fhirNutritionSupplement({ patientUserId: principal.userId, ...parsed.data });
