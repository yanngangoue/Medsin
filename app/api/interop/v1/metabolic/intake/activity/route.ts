...
import { activityIntakeSchema } from "@/lib/metabolic/schemas";
import { fhirObservationActivity } from "@/lib/metabolic/fhir-normalize";
import { persistMetabolicIntake, MetabolicConsentError } from "@/lib/metabolic/record-intake";

export async function POST(req: NextRequest) {
  return catchRouteError("interop/v1/metabolic/intake/activity/POST", async () => {
    if (!parseTenantFromHeaders(req.headers)) {
      ...
      const parsed = activityIntakeSchema.safeParse(json);
      if (!parsed.success) {
        return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
      }
      try {
        const fhir = fhirObservationActivity({ patientUserId: principal.userId, ...parsed.data });
