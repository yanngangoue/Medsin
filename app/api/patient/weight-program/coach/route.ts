...
export async function GET() {
  return catchRouteError("patient/weight-program/coach/GET", async () => {
...
export async function POST(req: Request) {
  return catchRouteError("patient/weight-program/coach/POST", async () => {
    const session = await auth();
      if (!session?.user?.id) {
        return NextResponse.json({ error: "Non autorisé", code: "UNAUTHORIZED" }, { status: 401 });
      }
      ...
      if (isDemoMode()) {
        const demoReply =
          "Merci pour ce partage. En mode démo, configurez ANTHROPIC_API_KEY pour des réponses personnalisées. Je suis Anne, votre coach IA — continuez vos bilans hebdomadaires réguliers, c'est la clé avec le GLP-1.";
