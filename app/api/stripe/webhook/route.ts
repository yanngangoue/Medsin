...
export async function POST(req: Request) {
  return catchRouteError("stripe/webhook/POST", async () => {
    const stripe = getStripe();
      const webhookSecret = process.env.STRIPE_WEBHOOK_SECRET;
      if (!webhookSecret) {
        return NextResponse.json({ error: "Webhook non configuré", code: "SERVICE_UNAVAILABLE" }, { status: 503 });
      }
    
      const signature = req.headers.get("stripe-signature");
      if (!signature) {
        return NextResponse.json({ error: "Signature manquante", code: "VALIDATION_ERROR" }, { status: 400 });
      }
      ...
      try {
        ...
        return NextResponse.json({ error: "Signature invalide", code: "VALIDATION_ERROR" }, { status: 400 });
      }
      ...
      try {
        ...
        return NextResponse.json({ error: "Traitement échoué", code: "INTERNAL_ERROR" }, { status: 500 });
      }
